...
function setup() {
  ...
  if (!salarySheet) {
    ...
    salarySheet.appendRow(['Timestamp', 'Name', 'Email', 'Amount', 'Reason', 'Status']);
  }
}
...
function handleSalaryAdvance(data) {
  const { name, email, amount, reason } = data;
  ...
  if (sheet) {
    sheet.appendRow([new Date(), name, email, amount, reason, 'Pending']);
  }
